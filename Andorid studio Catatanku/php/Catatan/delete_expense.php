...
// menghapus field dari table expense berdasarkan id expense
// <alamat ip server>/catatan/delete_expense.php?id=3
...
if(isset($_GET['id'])) {	
	...
	$check_id=mysqli_query($connect, "SELECT * FROM expense WHERE id_expense='$id'");
	$response = array();

	if($check_id){
		// jika data tersedia dalam database, lanjutkan dengan query delete
		$hasil_query=mysqli_query($connect, "DELETE FROM expense WHERE id_expense='$id'");
